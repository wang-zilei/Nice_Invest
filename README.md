...
- **MCP 工具链 + Function Call**：akshare（国内主力，东方财富/同花顺）+ Tushare Pro（第一备选）+ yfinance（海外兜底，Yahoo Finance）
...
│   │   ├── analyst.py      ← 基本面 Agent（akshare 主 → Tushare 备1 → yfinance 备2）
│   │   ├── technical.py    ← 技术面 Agent（akshare 主 → Tushare 备1 → yfinance 备2）
│   │   ├── valuation.py    ← 估值 Agent（akshare 主 → Tushare 备1 → yfinance 备2）
│   │   ├── news.py         ← 新闻 Agent（akshare 主 → yfinance 兜底）
│   │   └── summary.py      ← Summary Agent（ReAct，交叉验证 + 加权评分 + ReportData JSON）
│   └── mcp_tools/
│       ├── news_api.py     ← akshare 主力数据源（东方财富/同花顺，15s 超时保护）
│       ├── tushare_api.py  ← Tushare Pro 封装（第一备选）
│       ├── yahoo_api.py    ← yfinance 海外兜底（Yahoo Finance，国内正常时不需要）
