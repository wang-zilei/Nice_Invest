...
TECHNICAL_TOOLS = [get_technical_data_backup, get_technical_data]
...
1. **首选**调用 get_technical_data_backup 获取 akshare 日线行情（免费、不限流、60个交易日含MA均线统计）
2. 如需要更长的历史数据，再调用 get_technical_data 获取 Tushare 日线数据补充
3. 如 Tushare 限流或返回空数据，不影响分析——akshare 数据已足够覆盖趋势判断和关键价位识别
